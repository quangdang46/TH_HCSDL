...
--4.1
SELECT COUNT(*) AS TONGSV FROM DMSV
--4.2
SELECT GIOITINH=
  CASE
  ...
  ELSE 'NU'
  END
,COUNT(PHAI) FROM DMSV 
GROUP BY PHAI
--4.3

SELECT TENKHOA,COUNT(MASV) AS SOLUONGSV
FROM DMKHOA AS K,DMSV AS SV
WHERE K.MAKHOA=SV.MAKHOA
GROUP BY SV.MAKHOA,TENKHOA

--4.4
SELECT TENMH,COUNT(DISTINCT MASV) AS SOLUONG
FROM KETQUA AS KQ,DMMH AS MH
WHERE KQ.MAMH=MH.MAMH
GROUP BY TENMH,KQ.MAMH

--4.5
SELECT MASV,COUNT(DISTINCT KQ.MAMH) AS SOLUONG
FROM KETQUA AS KQ,DMMH AS MH
WHERE KQ.MAMH=MH.MAMH
GROUP BY MASV

--4.6
SELECT TENKHOA,MAX(HOCBONG)
FROM DMKHOA AS K,DMSV AS SV
WHERE K.MAKHOA=SV.MAKHOA
GROUP BY SV.MAKHOA,TENKHOA
...
--4.7
SELECT TENKHOA,SUM(
  CASE WHEN PHAI=0 THEN 1 ELSE 0 END
) AS NAM,
SUM(CASE WHEN PHAI=1 THEN 1 ELSE 0 END)
  AS NU
FROM DMKHOA AS K,DMSV AS SV
WHERE K.MAKHOA=SV.MAKHOA
...
--4.8
SELECT DATEDIFF(YEAR,NGAYSINH,GETDATE()) AS TUOI,COUNT(MASV) AS SOLUONG
FROM DMSV AS SV
